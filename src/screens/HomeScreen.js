...
const DashboardPage = (props) => {
  ...
  const qualities = [
    ...
    {
      title: "Expenses",
      caption: "Track expenses across all your different accounts.",
      img: brandImg,
    },
    {
      title: "24/7 expert support",
      caption: "Find deductions you didn’t know about to get all you deserve.",
      img: original,
    },
  ];
  ...
  }, [params, props.commonUserStatus]);

  useEffect(() => {
    if (props.isCreated || props?.headerName) {
      dispatch(getProduct(props?.headerName));
    }
  }, [props?.headerName, props?.isCreated]);
  ...
  useEffect(() => {
    if (getProductDetails && Object.keys(getProductDetails).length > 0) {
      if (
        ...
        Array.isArray(getProductDetails?.data) &&
        getProductDetails?.data.length > 0
      )
  ...
  const handleSubscription = (productId) => {
    ...
    if (!productValidation) {
      ...
      dispatch(
        alertActions.error(
          "Subscription not allowed at a moment.Please update your profile... "
        )
      );
      return;
    }
    if (
      ...
      storedData
        ? dispatch(
            alertActions.error(
              "Profile under verification.Try after sometime..."
            )
          )
    ...
    <div>
      ...
      <section className="div-center flex-column">
        <Grid
          paddingY={4}
          paddingX={12}
          className="div-center flex-column text-center"
        >
          <Typography variant="h5">Features that fit your business</Typography>
          <Typography variant="subtitle2">
            Lorem ipsum dolor sit amet. In fugit nihil id facere excepturi non
            officiis fugit vel saepe fugit sit autem fuga cum ratione galisum.
            Cum corrupti quam sit repellendus blanditiis sed asperiores
            veritatis 33 modi incidunt rem quia quibusdam.
          </Typography>
        </Grid>
        <Grid
          paddingY={4}
          ...
          container
          spacing={3}
          className="quality-section div-center text-center"
        >
          {/* description 1 */}
        ...
        <Grid container spacing={2} paddingY={4} paddingX={12}>
          <Grid xs={5} className="section-color shadow-sm rounded">
            <img
              alt=""
              className="rounded"
              src={productDetails?.image ?? account3}
            />
          </Grid>

          <Grid
            xs={7}
            className="div-center flex-column align-items-center section-color"
          >
            ...
            >
              <p>
                <b>Price</b> : {productDetails?.price ?? "100.00"}
              </p>
              <p>
                <b>Brand</b> : {productDetails?.Brand ?? "Apple"}
              </p>
            </Box>
            {subscribeProduct &&
            subscribeProduct.length > 0 &&
            subscribeProduct.includes(productDetails.id) ? (
              <Button
                variant="contained"
                color="error"
              ...
              <Button
                className="btn-color"
                sx={{ color: "#fff", marginBottom: "5px" }}
                onClick={() => handleSubscription(productDetails.id)}
              >
                Subscribe{" "}
              </Button>
            )}
